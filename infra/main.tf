...
locals {
  config = {
    defaults = {
      site        = "gillianbowling"
      domain_name = "gillianbowling.com"
    }
  ...
  bucket_name = "${local.deployment.domain_name}-${local.environment}-2"
...
resource "aws_s3_object" "upload_site" {
  ...
  for_each = fileset("../archive/", "**/*.*")

  key          = each.value
  source       = "../archive/${each.value}"
  content_type = lookup(tomap(local.mime_types), element(split(".", each.key), length(split(".", each.key)) - 1))
  etag         = filemd5("../archive/${each.value}")
}
